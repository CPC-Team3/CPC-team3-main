# This README file contains:
> 1. [Team Member Information](#Team-4-members)
> 2. [Instruction to setup the enviroment for the project](#setup-enviroment) (for each hometask and full simulation)
> > - [setup work space](#setup-workspace)
> > - [add project to work space](##-add-project-to-work-space)
> > - [setup library](#setup-library)
> 3. [Instruction to run the simulation](#run-full-simulation)
> 4. [Hometask from 1 to 5 for each member and final capstone project](#Home-task) (contains video link and tasks)
> > - [Hometask 1](#Home-task-1)
> > - [Hometask 2](#Home-task-2)
> > - [Hometask 3](#Home-task-3)
> > - [Hometask 4](#Home-task-4)
> > - [Hometask 5](#Home-task-5)
> > - [Final capstone project](#Final-capstone-project)
# Team 4 members
1. Sheikh Muhammad Adib bin Sh Abu Bakar, ESE Student, 7219310
2. Aditya Kumar, ESE Student, 7219675 
3. Mykyta Konakh, ESE Student, 7219011
4. Raed Kayali, ESE Student, 7219191

# setup enviroment
> ## setup workspace
> 1. clone this reprository to you machine (!do not download the zip file and unzip it because it will not work. instead, use git clone in your machine interminal)
> 2. open java eclipse
> 3. choose "CPC-team3-main" as work space when booting java

> ## add project to work space
> 1. open: file > import > General > existing project into workspce 
> 2. click next
> 3. in "select root directory" choose "capstone_project" folder (inside "CPC-team3-main" folder)
> 4. select (tick) capstone_project(<oath to the folder>)
> 5. click finish
> 6. close welcome window

> ## setup library
> 1. right click on the project 
> 2. select properties 
> 3. to "java build path" 
> 4. in  "java build path": go to "libraries tab"
> 5. in "libraries tab": dounle click on  "JRE System Library" (under "Modulepath")
> 6. select Alternate JRE
> 7. choose "jre" or other
> 8. click finish
> 9. click apply and close

# run full simulation
> 1. go to capstone_project > src > simulation
> 2. run Simulation.java

# Home task
- All diagrams are located [here](https://github.com/CPC-Team3/CPC-team3-main/blob/main/capstone_project/capstone_diagrams.drawio.pdf)
- System use case <br /> <img src='https://github.com/CPC-Team3/CPC-team3-main/blob/main/system_use_case.png' width='500'>

## Home task 1 
> ### Sheikh Muhammad Adib
> > - [Video](https://1drv.ms/v/s!AlQOuTxbuHLMkrtPOU6NlFzBZ_nKfQ?e=ReA000)
> > - Task:
> > > - Handling Multiple exceptions, Diagrams, Code structure, charging station, charger and system components  
> ### Aditya Kumar
> > - [video]()
> > - Task:
> > > - Re-throwing Exceptions, Diagram.
> ### Mykyta Konakh
> > - [video]()
> > - Task:
> > > - Task1 
> ### Raed Kayali
> > - [video]()
> > - Task:
> > > - Task1 
## Home task 2 
> ### Sheikh Muhammad Adib
> > - [Video](https://1drv.ms/v/s!AlQOuTxbuHLMkrwY7vUTQQ67MKRHvw?e=t0zmu3)
> > - Task:
> > > -	Managing the log files. Make log file for each component. 
> > > > -	System
> > > > -	Charging Station
> > > > -	Energy controller
> > > > -	Charger
> > > -	Create energy controller component (structure)
> > > -	Create car class for simulation
> > > > -	Car that can book
> > > > -	Car that can request without booking

> ### Aditya Kumar
> > - [video]()
> > - Task:
> > > - Booking slot log for car with higher priority  
> ### Mykyta Konakh
> > - [video]()
> > - Task:
> > > - Task1 
> ### Raed Kayali
> > - [video]()
> > - Task:
> > > - Task1 
## Home task 3 
> ### Sheikh Muhammad Adib
> > - [Video](https://1drv.ms/v/s!AlQOuTxbuHLMkrwaOVqcOtz1lxGILQ?e=RReu2y)
> > - Task:
> > > - GUI
> > > - simulate multiple station and charger
> > > - make car leave or change station after waiting for 15 min
> ### Aditya Kumar
> > - [video]()
> > - Task:
> > > - Task1 
> ### Mykyta Konakh
> > - [video]()
> > - Task:
> > > - Task1 
> ### Raed Kayali
> > - [video]()
> > - Task:
> > > - Task1 
## Home task 4 
> ### Sheikh Muhammad Adib
> > - [Video](https://1drv.ms/v/s!AlQOuTxbuHLMkrwbmK3zhPPku70wCg?e=pDHG2j)
> > - Task:
> > > - Add logs files for each day for each charging station, for each energy source and for all system as whole.
> ### Aditya Kumar
> > - [video]()
> > - Task:
> > > - Task1 
> ### Mykyta Konakh
> > - [video]()
> > - Task:
> > > - Task1 
> ### Raed Kayali
> > - [video]()
> > - Task:
> > > - Task1 
## Home task 5 
> ### Sheikh Muhammad Adib
> > - [Video](https://1drv.ms/v/s!AlQOuTxbuHLMkrwd-8sH6pIvC_HDbA?e=w6OtpT)
> > - Task:
> > > - 3 unit test for charger
> ### Aditya Kumar
> > - [video]()
> > - Task:
> > > - 1 unit test for addBookingSlot and 1 test for the charging station
> ### Mykyta Konakh
> > - [video]()
> > - Task:
> > > - Task1 
> ### Raed Kayali
> > - [video]()
> > - Task:
> > > - Task1
## Final capstone project
> > - [video1](https://1drv.ms/v/s!AlQOuTxbuHLMkr5DryMHTLuvAxlPnw?e=QTdpkx)

